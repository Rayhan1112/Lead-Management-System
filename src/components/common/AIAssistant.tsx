...
export const AIAssistant: React.FC = () => {
    ...
    const promptMessage: Message = {
      ...
      dropdown: {
        options: leads.map(lead => ({
          label: `${lead.firstName} ${lead.lastName} (${lead.status})`,
          value: lead.id || ''
        })),
        onSelect: (value) => {
          const selectedLead = leads.find(lead => lead.id === value);
          if (selectedLead) {
            addMessage(`Editing ${selectedLead.firstName} ${selectedLead.lastName}`, true);
    ...
    const promptMessage: Message = {
      ...
      dropdown: {
        ...
        onSelect: (value) => {
          const selectedLead = leads.find(lead => lead.id === value);
          if (selectedLead) {
            addMessage(`Delete ${selectedLead.firstName} ${selectedLead.lastName}?`, false);
            ...
            const confirmMessage: Message = {
              ...
              type: 'suggestion',
              action: () => {
                addMessage(`Yes, delete ${selectedLead.firstName} ${selectedLead.lastName}`, true);
  ...
  const showLeadList = () => {
    ...
    const leadMessages = leads.slice(0, 3).map(lead => ({
      id: `list-${lead.id}`,
      content: `${lead.firstName} ${lead.lastName} - ${lead.email} (${lead.status})`,
      sender: 'ai',
      timestamp: new Date(),
      type: 'contact',
      contact: {
  ...
  const showFilteredLeads = (filteredLeads: Lead[], status: string) => {
    ...
    const leadMessages = filteredLeads.slice(0, 5).map(lead => ({
      id: `filtered-${lead.id}`,
      content: `${lead.firstName} ${lead.lastName} - ${lead.email}`,
      sender: 'ai',
      timestamp: new Date(),
      type: 'contact',
      contact: {
  ...
  const showContactOptions = () => {
    ...
    const promptMessage: Message = {
      ...
      dropdown: {
        options: leads.map(lead => ({
          label: `${lead.firstName} ${lead.lastName} (${lead.phone || lead.email})`,
          value: lead.id || ''
        })),
        onSelect: (value) => {
          const selectedLead = leads.find(lead => lead.id === value);
  ...
  const showContactMethods = (lead: Lead) => {
    const promptMessage: Message = {
      ...
      content: `How would you like to contact ${lead.firstName} ${lead.lastName}?`,
      sender: 'ai',
      timestamp: new Date(),
    };
    ...
    if (lead.phone) {
      options.push({
        ...
        type: 'suggestion',
        action: () => {
          addMessage(`Calling ${lead.firstName} at ${lead.phone}`, true);
      ...
      },
      {
        ...
        type: 'suggestion',
        action: () => {
          addMessage(`Opening WhatsApp chat with ${lead.firstName}`, true);
    ...
    if (lead.email) {
      options.push({
        ...
        type: 'suggestion',
        action: () => {
          addMessage(`Emailing ${lead.firstName} at ${lead.email}`, true);
  ...
  const handleAddLead = async (lead: Lead) => {
    ...
    try {
      ...
      addMessage(`New lead created: ${lead.firstName} ${lead.lastName}`, false);
  ...
  const handleUpdateLead = async (updatedLead: Lead) => {
    ...
    try {
      ...
      setIsLeadFormOpen(false);
      addMessage(`Lead updated: ${updatedLead.firstName} ${updatedLead.lastName}`, false);
  ...
  const parseExcelData = (data: any[]): Lead[] => {
    return data.map((row, index) => ({
      firstName: row['First Name'] || row['firstName'] || `Imported ${index + 1}`,
      lastName: row['Last Name'] || row['lastName'] || '',
      email: row['Email'] || row['email'] || '',
      phone: row['Phone'] || row['phone'] || '',
      status: row['Status'] || row['status'] || 'new'
    })).filter(lead => lead.firstName && lead.email);
  };
